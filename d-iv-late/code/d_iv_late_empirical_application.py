...
def estimate_nuisance_functions_empirical(data, X_cols, y_grid, k_folds=5):
    ...
    for train_index, test_index in kf.split(data):
        ...
        pi_model = RandomForestClassifier(n_estimators=100, min_samples_leaf=10, random_state=42, class_weight='balanced')
        pi_model.fit(data_train[X_cols], data_train['Z'])
        pi_hat_oos[test_index] = pi_model.predict_proba(data_test[X_cols])[:, 1]
        
        # Estimate p(x, z) = P(W=1 | Z, X)
        p_model = RandomForestClassifier(n_estimators=100, min_samples_leaf=10, random_state=42, class_weight='balanced')
        p_model.fit(data_train[X_cols + ['Z']], data_train['W'])
        p_hat_oos[test_index] = p_model.predict_proba(data_test[X_cols + ['Z']])[:, 1]
        
        for i, y_val in enumerate(y_grid):
            data_train_y_loop = data_train.copy() # Use a copy for modifications inside the loop
            ...
            if len(data_train_y_loop['Y_le_y'].unique()) == 1:
                ...
                mu_y_model = RandomForestClassifier(n_estimators=100, min_samples_leaf=10, random_state=42, class_weight='balanced')
                mu_y_model.fit(data_train_y_loop[X_cols + ['W']], data_train_y_loop['Y_le_y'])
                mu_hat_y_oos[test_index, i] = mu_y_model.predict_proba(data_test[X_cols + ['W']])[:, 1]

            # Estimate E[1{Y<=y} | X, W=1]
            data_train_y_w1 = data_train_y_loop[data_train_y_loop['W'] == 1]
            if len(data_train_y_w1) < 2 or len(data_train_y_w1['Y_le_y'].unique()) == 1: # Need at least 2 samples for RF and multiple classes
                # Fallback: predict based on overall proportion in train or a default (e.g., 0.5)
                # Or use proportion from data_train_y_loop['Y_le_y'] if W=1
                ...
                mu_y_w1_model = RandomForestClassifier(n_estimators=100, min_samples_leaf=10, random_state=42, class_weight='balanced')
                mu_y_w1_model.fit(data_train_y_w1[X_cols], data_train_y_w1['Y_le_y'])
                mu_hat_y_w1_oos[test_index, i] = mu_y_w1_model.predict_proba(data_test[X_cols])[:, 1]
            ...
            if len(data_train_y_w0) < 2 or len(data_train_y_w0['Y_le_y'].unique()) == 1:
                ...
                mu_y_w0_model = RandomForestClassifier(n_estimators=100, min_samples_leaf=10, random_state=42, class_weight='balanced')
                mu_y_w0_model.fit(data_train_y_w0[X_cols], data_train_y_w0['Y_le_y'])
                mu_hat_y_w0_oos[test_index, i] = mu_y_w0_model.predict_proba(data_test[X_cols])[:, 1]
...
def main():
    ...
    # Ensure D-LATE directory exists for outputs
    output_dir = "D-LATE"
    ...
    results_csv_path = os.path.join(output_dir, 'empirical_dlate_results.csv')
    results_df.to_csv(results_csv_path, index=False)
    print(f"\nEmpirical D-LATE results saved to {results_csv_path}")

    plot_path = os.path.join(output_dir, 'empirical_dlate_plot.png')
    plt.figure(figsize=(10, 6))
    plt.plot(y_grid, dlate_estimates, marker='o', linestyle='-')
    plt.title('Estimated Distributional LATE (D-LATE) - Pension Data')
